...
def acceptInvitation(request, token):
    ...
    if not invite:
        return cors_json({"error": "Invalid token"}, status=400)
    ...
    if user:
        ...
        return cors_json({"message": "Joined organisation"}, status=200)

    return cors_json({"status": "new-user", "email": invite.email}, status=200)
@csrf_exempt
